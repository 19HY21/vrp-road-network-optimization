...
def _check(strategy: str, name: str, passed: bool, detail: str = "") -> dict:
    status = "✅ PASS" if passed else "❌ FAIL"
    print(f"  [{strategy}] {status}  {name}{('  ' + detail) if detail else ''}")
    return {"strategy": strategy, "check": name, "status": "PASS" if passed else "FAIL", "detail": detail}
...
def _evaluate_one_strategy(
    ...
    vehicles_used = int(strategy_summary["vehicles_used"])
    total_dist = float(strategy_summary["total_dist_km"])
    total_cost = float(strategy_summary["total_cost_yen"])
    results = []

    # 1. 全配送先訪問
    visited_ids = set(dest_routes["location_id"].tolist())
    missing = valid_ids - visited_ids
    results.append(_check(strategy, "全配送先訪問", len(missing) == 0,
        f"未訪問: {len(missing)} 件" if missing else f"対象 {len(valid_ids)} 件すべて訪問済み"))

    # 2. 重複なし
    dup_count = dest_routes["location_id"].duplicated().sum()
    results.append(_check(strategy, "各配送先1回のみ訪問", dup_count == 0,
    ...
    results.append(_check(strategy, "積載上限", len(over_capacity) == 0,
    ...
    # 4. 時間帯・午前（13:00以前に到着）
    if "time_slot_ok" in dest_routes.columns and "time_slot_code" in dest_routes.columns:
        morning_stops = dest_routes[dest_routes["time_slot_code"] == 1]
        late_morning = morning_stops[morning_stops["time_slot_ok"] == False]
    else:
    ...
    results.append(_check(strategy, "時間帯制約・午前（13:00まで）", len(late_morning) == 0,
        f"遅延: {len(late_morning)} 件" if len(late_morning) > 0 else f"午前指定 {len(morning_stops)} 件すべて充足"))

    # 5. 時間帯・午後（13:00以降に到着）
    if "time_slot_ok" in dest_routes.columns and "time_slot_code" in dest_routes.columns:
        afternoon_stops = dest_routes[dest_routes["time_slot_code"] == 2]
        early_afternoon = afternoon_stops[afternoon_stops["time_slot_ok"] == False]
    else:
        ...
        early_afternoon = afternoon_stops[afternoon_stops["arrival_min"] < morning_end_min]
    results.append(_check(strategy, "時間帯制約・午後（13:00以降）", len(early_afternoon) == 0,
    ...
    results.append(_check(strategy, f"業務終了時刻（{work_end_label}帰着）", len(late_returns) == 0,
    ...
    results.append(_check(strategy, "デポ出発・帰着", valid_depot, "全車両がデポ始発・終着"))

    # 8. 使用台数制限（≤ vehicle_count）
    results.append(_check(strategy, f"使用台数 ≤ 上限({v_limit}台)", vehicles_used <= v_limit,
    ...
    results.append(_check(strategy, "コスト内訳整合", cost_match,
...
def run_evaluation(plan_id: str, depot_id: str | None = None, input_stem: str | None = None) -> pd.DataFrame:
    ...
    all_results = []
    for _, strategy_row in solved_summary.iterrows():
        strategy = str(strategy_row["strategy"])
        routes = detail_df[detail_df["strategy"] == strategy]
        dest_routes = routes[routes["location_type"] == "destination"]
        print(f"\n=== 制約検証 [{strategy}] (plan_id: {plan_id} / 使用台数: {int(strategy_row['vehicles_used'])} 台) ===")
        all_results.extend(_evaluate_one_strategy(
            strategy, strategy_row, routes, dest_routes, valid_ids, txn_df,
            morning_end_min, work_end_min, work_end_label,
            capacity, v_limit, fixed_cost, dist_unit,
        ))
...
def main(plan_id: str | None = None, depot_id: str | None = None, input_stem: str | None = None) -> None:
    ...
    for strategy, grp in results_df.groupby("strategy"):
        ...
        f = (grp["status"] == "FAIL").sum()
        print(f"  戦略 {strategy}: {p} PASS / {f} FAIL")
    pass_count = (results_df["status"] == "PASS").sum()
    fail_count = (results_df["status"] == "FAIL").sum()
    print(f"合計: {pass_count} PASS / {fail_count} FAIL（全{len(results_df)}項目）")
